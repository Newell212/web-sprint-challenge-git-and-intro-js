...
function getArtistByIndex(artArr, desInd) {
  return `the artist at index ${artArr[desInd].id} is ${artArr[desInd].name}`
}
...
function removeArtist(artArr, artInd) {
  const updArr = [...artArr]

  updArr.splice(artInd, 1);
...
function addArtist(artArr, name, years, genre, nationality, bio) {
  let cpyArr = [...artArr]
...
function lotsOfArt(artArr) {
  ...
  for (let i = 0; i < artArr.length; i++) {
    if(artArr[i].paintings > 100) {
      let paint = artArr[i].name
      newArr.push(paint);
    }
    //newArr =artArr[i].name
  }
  return newArr
}
...
function artistInfo(artArr, artName){
  let artBio = "";
  for (let i = 0; i < artArr.length; i++) {
    if (artArr[i].name === artName) {
      artBio = artArr[i].bio
...
function artistByCountry(artArr, artNat){
  let natArr =[];
  for (let i = 0; i < artArr.length; i++) {
      if (artArr[i].nationality === artNat) {
        let homeNat = artArr[i].name
        natArr.push(homeNat)
      }
  }
  return natArr
